...
/**
 * An instrumentation test class for [com.example.network.datasource.coins.CoinsDataSource].
...
class CoinsDataSourceTest {
    ...
    /**
     * Injects an instance of [com.example.network.datasource.coins.CoinsDataSource] provided by Hilt.
    ...
    fun getCoinMarketsList_showDataInLog_returnsNonEmptyList() = runTest {
        try {
            ...
            val result =
                remoteDataSource.getCoinMarkets("usd", page = 1, perPage = 50, sparkline = false)

            // Log the fetched data for manual verification.
            val formattedJson = json.encodeToString(result)
            Timber.Forest.d("--- Fetched Data ---\n%s", formattedJson)
