...
class TrendingMapperTest {
    ...
    @Test fun `maps populated dto`() {
        val dto = TrendingDto(
            categories = listOf(TrendingDto.Category(
                coinsCount = 5,
        ...
        assertThat(domain.coins!!.first().item?.name).isEqualTo("Bitcoin")
    ...
    @Test fun `maps null fields safely`() {
        ...
        assertThat(domain?.categories).isNull()
        assertThat(domain?.coins).isNull()
        assertThat(domain?.nfts).isNull()
    }
}
