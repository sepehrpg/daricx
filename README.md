...
- **Data layer** – repositories, local (Room/DataStore) + remote (~~Retrofit~~ **Ktor**) sources (the Retrofit-based implementation is kept in the `baseline/retrofit-hilt` branch).
...
- ~~Retrofit & OkHttp~~ (legacy networking stack, preserved on `baseline/retrofit-hilt`)
- **Ktor** HTTP client (in `core:network`) – current networking stack, used for crypto APIs and future KMP support
